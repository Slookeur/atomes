  ...
  if (wingl -> init)
  {
    ...
    if (h)
    {
      ...
      if (ptot > 0)
      {
        ...
        wingl -> ogl_glsl[POLYS][step][0] = init_shader_program (POLYS, GLSL_POLYEDRA, (plot -> ray_tracing) ? full_vertex_ray : full_vertex, NULL,
                                                                                       (plot -> ray_tracing) ? full_color_ray : full_color, GL_TRIANGLES, 3, 1, TRUE, poly);
        g_free (poly);
      }
    }
  }
