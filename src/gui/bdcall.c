...
#else
  active_project -> min[BND] = * min;
  active_project -> max[BND] = * max;
#endif
}
  ...
  for ( i = 0 ; i < active_project -> nspec ; i++ )
  {
    for ( j = 0 ; j < active_project -> nspec ; j++ )
    {
      ...
      active_project -> analysis[BND] -> curves[k] -> name = g_strdup_printf("Dij [%s-%s]",
                                                         ...
                                                         active_chem -> label[j]);
#else
      active_project -> curves[BND][k] -> name = g_strdup_printf("Dij [%s-%s]",
                                                active_chem -> label[i],
                                                active_chem -> label[j]);
#endif
      k=k+1;
...
#else
  active_project -> initok[BND] = TRUE;
#endif
}
    ...
    for ( j = 0 ; j < active_project -> nspec ; j++ )
    {
      for ( k = 0 ; k < active_project -> nspec ; k++ )
      {
        ...
        active_project -> analysis[ANG] -> curves[h] -> name = g_strdup_printf("Angles [%s-%s-%s]",
                                                                           ...
                                                                           active_chem -> label[k]);
#else
        active_project -> curves[ANG][h] -> name = g_strdup_printf("Angles [%s-%s-%s]",
                                                  active_chem -> label[i],
                                                  active_chem -> label[j],
                                                  active_chem -> label[k]);
#endif
...
#else
          active_project -> curves[ANG][h] -> name = g_strdup_printf("Dihedral [%s-%s-%s-%s]",
                                                    active_chem -> label[i], active_chem -> label[j],
                                                    active_chem -> label[k], active_chem -> label[l]);
#endif
          h=h+1;
...
#else
  active_project -> initok[ANG] = TRUE;
#endif
}
...
void update_ang_view (project * this_proj)
{
  ...
  str = g_strdup_printf ("%d", this_proj -> num_delta[ANG]);
  ...
  str = g_strdup_printf ("%f", this_proj -> delta[ANG]);
  print_info (str, "bold_blue", this_proj -> text_buffer[AN+OT]);
  g_free (str);
  print_info (" °\n", "bold", this_proj -> text_buffer[AN+OT]);
  print_info (calculation_time(TRUE, this_proj -> calc_time[ANG]), NULL, this_proj -> text_buffer[AN+OT]);
#endif
}
...
G_MODULE_EXPORT void on_calc_bonds_released (GtkWidget * widg, gpointer data)
{
...
#else
  gboolean vis_bd = active_project -> visok[BND];
#endif
  ...
  if (active_project -> dmtx)
  {
    ...
    if (bonds_update || active_project -> runc[0] || active_project -> runc[2])
    {
...
#else
      if (! active_project -> initok[BND] && bonding) initbd ();
      if (active_project -> runc[0]) clean_curves_data (BND, 0, active_project -> numc[BND]);
#endif
...
#else
      active_project -> delta[BND] = (active_project -> max[BND]-active_project -> min[BND]) / active_project -> num_delta[BND];
      clock_gettime (CLOCK_MONOTONIC, & start_time);
      j = bonding_ (& m, & l, & bonding, & active_project -> num_delta[BND], & active_project -> min[BND], & active_project -> delta[BND], active_project -> bondfile);
      clock_gettime (CLOCK_MONOTONIC, & stop_time);
      active_project -> calc_time[BND] = get_calc_time (start_time, stop_time);
      active_project -> runok[SPH] = j;
#endif
      ...
      {
#ifdef NEW_ANA
        if (active_glwin -> init) print_info (calculation_time(TRUE, active_project -> analysis[BND] -> calc_time), NULL, active_project -> analysis[BND] -> calc_buffer);
#else
        if (active_glwin -> init) print_info (calculation_time(TRUE, active_project -> calc_time[BND]), NULL, active_project -> text_buffer[BD+OT]);
#endif
        bonding = 1;
        if (frag_update)
        {
...
#else
          active_project -> calc_time[RIN] = get_calc_time (start_time, stop_time);
#endif
...
#else
      if (! active_project -> initok[ANG]) initang ();
      clean_curves_data (ANG, 0, active_project -> numc[ANG]);
      prepostcalc (widg, FALSE, ANG, statusb, opac);
      active_project -> delta[ANG] = 180.0 / active_project -> num_delta[ANG];
      clock_gettime (CLOCK_MONOTONIC, & start_time);
      j = bond_angles_ (& active_project -> num_delta[ANG]);
#endif
      if (! j)
      {
...
#else
        j = bond_diedrals_ (& active_project -> num_delta[ANG]);
        clock_gettime (CLOCK_MONOTONIC, & stop_time);
        active_project -> calc_time[ANG] = get_calc_time (start_time, stop_time);
#endif
        if (! j)
        {
